...
//    int64_t *new_indices = calculate_positions(0, num_vertices, gaps);
    ...
    // count all edges + vertices moved during resize
    num_read_resize_ += (num_vertices + num_edges_);
    num_write_resize_ += (num_vertices + num_edges_);
  ...
  void insert(int32_t src, int32_t dst, int32_t value)
  {
    ...
    if (right_free_slot != -1)
    {
      ...
      {
        // count edge moves
        num_write_insert_ += (right_free_slot - loc);
        num_read_insert_ += (right_free_slot - loc);
      ...
      num_write_insert_ += 1;  // new edge write

      vertices_[src].degree += 1;
      num_write_insert_ += 1;  // degree update
      num_read_insert_ += 1;
    }
    else if (left_free_slot != -1)
    {
      ...
      {
        // count edge moves
        num_write_insert_ += (loc - 1 - left_free_slot);
        num_read_insert_ += (loc - 1 - left_free_slot);
...
//    int64_t *new_index = calculate_positions(start_vertex, end_vertex, gaps);
    ...
    while (curr_vertex < end_vertex)
    {
      ...
      if(new_index[ii-start_vertex] <= vertices_[ii].index) {
        ...
        num_read_rebal_ += 2;  // read index + degree

        num_write_rebal_ += (last_read_index - read_index);  // edge copies
        num_read_rebal_ += (last_read_index - read_index);
        ...
        num_write_rebal_ += 1;  // vertex metadata update
      ...
      for (jj=ii; jj>=curr_vertex; jj-=1)
      {
        ...
        num_write_rebal_ += (read_index - last_read_index + 1);  // edge copies
        num_read_rebal_ += (read_index - last_read_index + 1);
        while(read_index >= last_read_index)
        {
          edges_[write_index] = edges_[read_index];
          //edges_[read_index].v = 0;
...
private:
  ...
  /* Performance counters (matching Python VCSR) */
  int64_t num_write_insert_ = 0;
  int64_t num_read_insert_ = 0;
  int64_t num_write_rebal_ = 0;
  int64_t num_read_rebal_ = 0;
  int64_t num_write_resize_ = 0;
  int64_t num_read_resize_ = 0;
  int64_t num_resize_ = 0;
  int64_t num_rebalance_ = 0;

public:
  void PrintMetrics() const {
    int64_t total_writes = num_write_insert_ + num_write_rebal_ + num_write_resize_;
    int64_t total_reads = num_read_insert_ + num_read_rebal_ + num_read_resize_;
    cout << "METRICS_START" << endl;
    cout << "total_writes " << total_writes << endl;
    cout << "total_reads " << total_reads << endl;
    cout << "num_write_insert " << num_write_insert_ << endl;
    cout << "num_read_insert " << num_read_insert_ << endl;
    cout << "num_write_rebal " << num_write_rebal_ << endl;
    cout << "num_read_rebal " << num_read_rebal_ << endl;
    cout << "num_write_resize " << num_write_resize_ << endl;
    cout << "num_read_resize " << num_read_resize_ << endl;
    cout << "num_resize " << num_resize_ << endl;
    cout << "num_rebalance " << num_rebalance_ << endl;
